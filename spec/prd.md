...
## 九、區域解鎖與等級系統 (Region Unlock & Leveling)

### 1. 區域解鎖機制

#### 1.1 雙重門檻系統

每個測驗區域的解鎖需**同時滿足**以下兩個條件：

1. **先決測驗完成（Prerequisite Quest）**
2. **玩家等級達標（Unlock Level）**

**公式**：
```
區域可用 = (先決測驗已完成) AND (玩家等級 >= 解鎖等級)
```

#### 1.2 區域配置表

| 區域 ID | 區域名稱 | 解鎖等級 | 先決條件 | 解鎖獎勵 |
|---------|----------|----------|----------|----------|
| `mbti` | MBTI 聖殿 | Lv.1 | 無 | 核心職業 (Class) |
| `big_five` | Big Five 能量場 | Lv.2 | 完成 MBTI 聖殿 | 角色屬性 (Stats) |
| `enneagram` | Enneagram 冥想塔 | Lv.3 | 完成 Big Five 能量場 | 靈魂種族 (Race) |
| `disc` | DISC 戰鬥叢林 | Lv.4 | 完成 Enneagram 冥想塔 | 對戰方式 (Stance) |
| `gallup` | Gallup 祭壇 | Lv.5 | 完成 DISC 戰鬥叢林 | 傳奇技能 (Talent) |

#### 1.3 解鎖順序

系統採用**嚴格線性順序解鎖**，玩家必須按照以下路徑推進：

```
MBTI 聖殿 → Big Five 能量場 → Enneagram 冥想塔 → DISC 戰鬥叢林 → Gallup 祭壇
```
...
| `AVAILABLE` | 已解鎖可進入 | 正常顯示，可點擊進入 |
| `LOCKED` | 尚未解鎖 | 灰階顯示 + 覆蓋迷霧 + 鎖鏈圖示 |

#### 1.5 鎖定提示優先級

當區域處於 `LOCKED` 狀態時，系統依序檢查並顯示：

1. **先決條件未滿足** → 「需先完成【前置區域名稱】試煉」
2. **等級不足** → 「需達到等級 Lv.X」
...
- **完成測驗題目**：基礎 10 EXP/題
- **高品質回答加成**：1.2x ~ 2.0x（由 Analytics Agent 評估）
- **完成整個測驗**：額外獎勵 EXP

**總公式**：
```
Total_EXP = Σ(每題基礎 EXP × 品質加成) + 完成獎勵
```
...
| 等級 | 解鎖功能 | 影響 |
|------|----------|------|
| Lv.1 | 新手冒險者 | 可進入 MBTI 聖殿 |
| Lv.2+ | 進階試煉 | 逐步解鎖後續區域 |
| **Lv.11** | **靈魂對話模式** | **解鎖開放式文字輸入**，玩家可用自然語言回答，AI 進行語義解析並給予更高品質 EXP |
| Lv.20 | 深邃試煉 | 允許更長的測驗流程（題數增加） |

> **關鍵機制**：Lv.11 靈魂對話模式是核心分水嶺，從「量化試煉」升級為「靈魂對話」，大幅提升敘事深度與個性化分析精度。

#### 2.4 等級與解鎖的關係

**等級的作用**：
- 作為區域解鎖的**必要條件之一**（但非充分條件）
- 解鎖特殊功能（如 Lv.11 靈魂對話）
- 象徵玩家在 TraitQuest 世界的資歷

**重要釐清**：
- ❌ **錯誤理解**：達到 Lv.5 就能直接玩 Gallup
- ✅ **正確理解**：達到 Lv.5 **且** 完成前四個測驗，才能解鎖 Gallup
...
- **CONQUERED**：區域發光、顯示勳章圖示
- **AVAILABLE**：正常渲染、可點擊、懸停顯示區域描述
- **LOCKED**：灰階 + 迷霧效果 + 鎖鏈圖示 + Tooltip 顯示 `unlock_hint`
