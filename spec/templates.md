...
- 延續性：必須讀取冒險者的 hero_chronicle，在開場白中提到他們過去的行為（例如：「我記得你曾選擇在森林中保護那隻幼獸...」）。
  這份 hero_chronicle 是由 Summary Agent (史官) 壓縮的語義摘要，讓你即使在第 10 題也能知道玩家在第 1 題的關鍵抉擇。
...
2. **撈取歷史紀錄**:
   ...
   - 從 **PostgreSQL** 的 `user_quests.interactions` (JSONB) 中讀取完整的歷史對話與分析標籤。

3. **讀取長期記憶**: 從資料庫中讀取最新的 `hero_chronicle`(由 Summary Agent 生成的語義摘要)。

4. **並行執行** (關鍵優化):
   
   ```python
   # 偽代碼示意
   async def handle_submit_answer(session_id, answer):
       # 並行啟動兩個任務
       next_q_task = asyncio.create_task(
           questionnaire_agent.generate_next_question(session_id, hero_chronicle)
       )
       
       analysis_task = asyncio.create_task(
           analytics_agent.analyze_and_save(session_id, answer)
       )
       
       # 立即等待並返回下一題
       next_question = await next_q_task
...
5. **數據聚合** (測驗結束時):
   ...
   ```python
   async def handle_quest_complete(session_id):
       # 等待所有非同步分析任務完成
       await asyncio.gather(*pending_analysis_tasks[session_id])
       
       # 聚合所有 trait_deltas
       accumulated_tags = await aggregate_trait_deltas(session_id)
       
       # 執行 Transformation Agent
       result = await transformation_agent.transform(accumulated_tags)
...
6. **更新快取**: 將當前輪次的對話寫入 Redis 快取,並增量更新 PostgreSQL。
...
pending_analysis_tasks = {
    "session_id_1": [task1, task2, task3],
    "session_id_2": [task1, task2],
    ...
}
```

當測驗結束時,使用 `asyncio.gather` 確保所有任務完成。
...
這種架構將「AI 的短期記憶 (Prompt)」轉化為「系統的長期記憶 (DB)」,使得即使 WebSocket 是事件驅動的,玩家感受到的卻是一個具備完整記憶、能根據過往選擇給予回饋的連續遊戲體驗。
